...
//go:generate cbor-gen-for FundedAddressState

// This is losely copied from the lotus market fund manager

// FundManagerAPI is the specific methods called by the FundManager
// (used by the tests)
type FundManagerAPI interface {
	MpoolPushMessage(context.Context, *fil.Message, *MessageSendSpec) (*fil.SignedMessage, error)
	StateMarketBalance(context.Context, address.Address, fil.TipSetKey) (fil.MarketBalance, error)
	StateWaitMsg(ctx context.Context, cid cid.Cid, confidence uint64) (*fil.MsgLookup, error)
}
...
func (a *fundManagerAPI) StateWaitMsg(ctx context.Context, c cid.Cid, conf uint64) (*fil.MsgLookup, error) {
	return a.api.StateWaitMsg(ctx, c, conf)
}
...
func NewFundManager(ds datastore.Batching, fapi fil.API, w wallet.Driver) *FundManager {
	ctx, cancel := context.WithCancel(context.Background())
	api := &fundManagerAPI{fapi, w}
	return &FundManager{
		ctx:         ctx,
		shutdown:    cancel,
		api:         api,
...
func (a *fundedAddress) reserve(ctx context.Context, addr address.Address, amt abi.TokenAmount) (cid.Cid, error) {
	return a.requestAndWait(ctx, addr, amt, &a.reservations)
}
...
func (a *fundedAddress) withdraw(ctx context.Context, addr address.Address, amt abi.TokenAmount) (cid.Cid, error) {
	return a.requestAndWait(ctx, addr, amt, &a.withdrawals)
}
...
func (a *fundedAddress) process() {
	...
	if haveWithdrawals && a.state.MsgCid == nil && len(a.reservations) == 0 {
		...
		if err == nil && withdrawalCid != cid.Undef {
			a.applyStateChange(&withdrawalCid, fil.EmptyInt)
		}
...
func (a *fundedAddress) saveState() {
	...
	if err != nil {
		fmt.Printf("saving state to store for addr %s: %v\n", a.state.Addr, err)
	}
}
...
func (a *fundedAddress) processReservations(reservations []*fundRequest, releases []*fundRequest) (pr *processResult, prerr error) {
	...
	reserved := fil.BigAdd(a.state.AmtReserved, reservedDelta)
	...
	if len(toAdd) > 0 && reserved.GreaterThan(abi.NewTokenAmount(0)) {
		...
		amtToAdd = fil.BigSub(reserved, avail)
...
func splitReservations(reservations []*fundRequest, releases []*fundRequest) ([]*fundRequest, []*fundRequest, abi.TokenAmount) {
	...
	for _, req := range releases {
		releaseAmt = fil.BigAdd(releaseAmt, req.Amount())
	}
	...
	for _, req := range reservations {
		...
		if amt.LessThanEqual(releaseAmt) {
			...
			releaseAmt = fil.BigSub(releaseAmt, amt)
		...
		if batchWallet == req.Wallet {
			delta := fil.BigSub(amt, releaseAmt)
			toAddAmt = fil.BigAdd(toAddAmt, delta)
			releaseAmt = abi.NewTokenAmount(0)
	...
	reservedDelta := fil.BigSub(toAddAmt, releaseAmt)
...
func (a *fundedAddress) processWithdrawals(withdrawals []*fundRequest) (msgCid cid.Cid, prerr error) {
	...
	netAvail := fil.BigSub(avail, a.state.AmtReserved)
	...
	for _, req := range withdrawals {
		...
		newWithdrawalAmt := fil.BigAdd(withdrawalAmt, amt)
		if newWithdrawalAmt.GreaterThan(netAvail) {
			msg := fmt.Sprintf("insufficient funds for withdrawal of %s: ", fil.FIL(amt))
			msg += fmt.Sprintf("net available (%s) = available (%s) - reserved (%s)",
				fil.FIL(fil.BigSub(netAvail, withdrawalAmt)), fil.FIL(avail), fil.FIL(a.state.AmtReserved))
			if !withdrawalAmt.IsZero() {
				msg += fmt.Sprintf(" - queued withdrawals (%s)", fil.FIL(withdrawalAmt))
			}
			err := fmt.Errorf(msg)
			a.debugf("%s", err)
			req.Complete(cid.Undef, err)
		...
		allowedAmt = fil.BigAdd(allowedAmt, amt)
	}
...
func (a *fundedAddress) startWaitForResults(msgCid cid.Cid) {
	go func() {
		...
		if err != nil {
			...
			fmt.Printf("waiting for results of message %s for addr %s: %v\n", msgCid, a.state.Addr, err)
		}
...
func (a *fundedAddress) debugf(args ...interface{}) {
	...
	args = args[1:]
	fmt.Printf(a.state.Addr.String()+": "+fmtStr+"\n", args...)
}
...
func (env *fundManagerEnvironment) AvailableFunds(ctx context.Context, addr address.Address) (abi.TokenAmount, error) {
	bal, err := env.api.StateMarketBalance(ctx, addr, fil.EmptyTSK)
	if err != nil {
		return abi.NewTokenAmount(0), err
	}

	return fil.BigSub(bal.Escrow, bal.Locked), nil
}
...
func (env *fundManagerEnvironment) AddFunds(
	...
	params, err := SerializeParams(&addr)
	if err != nil {
		return cid.Undef, err
	}

	smsg, aerr := env.api.MpoolPushMessage(ctx, &fil.Message{
...
func (env *fundManagerEnvironment) WithdrawFunds(
	...
	params, err := SerializeParams(&market.WithdrawBalanceParams{
...
func (env *fundManagerEnvironment) WaitMsg(ctx context.Context, c cid.Cid) error {
	_, err := env.api.StateWaitMsg(ctx, c, uint64(5))
	return err
}

const dsKeyAddr = "Addr"

// Store just persists storage fund states
type Store struct {
...
// TODO: duplicated in payments message builder
