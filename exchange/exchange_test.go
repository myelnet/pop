...
func TestExchangeE2E(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		t.Run(fmt.Sprintf("Try %v", i), func(t *testing.T) {
			...
			require.NoError(t, client.wdg.Index().SetRef(&DataRef{
				PayloadCID:  rootCid,
				StoreID:     storeID,
